...
class ReadFileCommand(Command):
    ...
    def execute(self, state: SessionState):
        ...
        if state.data_file_type == DataFileType.DUMP_TXT:
            state.header = dumptxt.read_header(data_file_name)
            state.atoms = dumptxt.read_atoms(data_file_name, state.atom_data_columns)
        elif state.data_file_type == DataFileType.WRITE_DATA:
            state.header = writedata.read_header(data_file_name)
            state.atoms = writedata.read_atoms(data_file_name, state.atom_data_columns)
