    ...
    def __init__(self, command_name: str, args: list[str]):
        ...
        if self._filter_type == FilterType.ATOM_TYPE:
            ...
            if len(self._filter_params) != 5:
                raise Exception("'neighbor_count' filter requires exactly five parameters: filter name for central atoms, filter name for neighbor atoms, neighbor count min (int or 'none'), neighbor count max (int or 'none'), cutoff distance (float)")
    ...
    def execute(self, state: SessionState):
        if self._filter_type == FilterType.ATOM_TYPE:
            ...
            if filter_name_central_atoms not in state.filters:
                raise Exception(f"'neighbor_count' filter parameter for central atoms '{filter_name_central_atoms}' is not the name of an existing filter")
            if filter_name_neighbor_atoms not in state.filters:
                raise Exception(f"'neighbor_count' filter parameter for neighbor atoms '{filter_name_neighbor_atoms}' is not the name of an existing filter")
