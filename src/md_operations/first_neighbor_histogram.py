...
def build_first_neighbor_histogram_data(atoms_center: list[Atom], atoms_neighbor: list[Atom], r_cutoff: float) -> dict[int, float]:
    ...
    for atom_center in atoms_center:
        neighbor_count: int = sum(1 for atom_neighbor in atoms_neighbor if (atom_center.pos.x - atom_neighbor.pos.x)**2 + (atom_center.pos.y - atom_neighbor.pos.y)**2 + (atom_center.pos.z - atom_neighbor.pos.z)**2 <= r_cutoff_sq)
